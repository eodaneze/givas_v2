...
let hosted_url = "https://givas.org";

let isLocal = window.location.origin.includes('localhost') || window.location.origin.includes('127.0.0.1');
let base_url = isLocal ? local_url : hosted_url;
...
function approveDocs(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
          ...
          window.location.href = `${base_url}/admin/inc/approveCharityFundDocs?id=${id}`;
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          ...
          if (emailResult.isConfirmed) {
            ...
            setTimeout(() => {
              fetch(
                `${base_url}/admin/inc/rejectCharityFundDocs?id=${id}`,
                {
                  method: "POST",
                  headers: {
